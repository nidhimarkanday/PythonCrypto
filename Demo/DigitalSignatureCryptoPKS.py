...
def verify_signature(key, data, sig_f):
    ...
    rsp = True if (signer.verify(h, signature))  else False 
    return rsp
...
if (op == "-s"):
    ...
    str = f"Generate Signature for Alice using : {newline} 1.PrivateKey {newline} 2.Data {newline} ************************************************************************************"
    print(str)
    generate_signature(key, data, sig_f)
    print(newline)
    print('RESULT: Signature is stored in  File')
elif (op == "-v"):
    # Verify Signature
    str=f"{newline}**********************************************Tom needs to verify data & DigitalSignature using*************************{newline}1.PublicKey received from Alice {newline}2.Data (ideally hash of data) {newline}3.DigitalSignature"
    print(str)
    print(newline)
